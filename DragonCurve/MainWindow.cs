...
namespace DragonCurve
{
    public partial class MainWindow : Form
    {
        ...
        private void drawBt_Click(object sender, EventArgs e)
        {
            ...
            if (dragonFractal.Checked)
            {
                var pixels = new Pixels();
                var image = new Bitmap(600, 400, PixelFormat.Format24bppRgb);
                using (var g = Graphics.FromImage(image))
                {
                    g.Clear(Color.Black);
                }

                DragonFractal.DrawDragonFractal(pixels, valueBar.Value * 10000, 123456);
                pixels.DrawToBitmap(image);

                pictureBox.Image = image;

                image.Save(path + "dragon.png", ImageFormat.Png);
            } 
            else if (dragonCurve.Checked)
            {
        ...
        private void saveToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (dragonFractal.Checked)
            {
                ...
                //сохранение в Word
                string imageDoc = path + "dragon.png";
                string doc = path + "DragonFractal.docx";
                ...
                string excelSave = path + "Dragon.xls";
